...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_bagtubuh&judulx=Bagian Tubuh" target="_blank">Cetak Semua</a>
              ...
              <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  ...
                  <div class="controls span7">
                    <input id="cname" name="nama_bagtubuhTB" placeholder="nama bagtubuh (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan bagtubuh (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Bagian Tubuh </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_bagtubuh" selected="selected">nama bagtubuh</option>
            ...
            <table id="datatable_examplez" class="responsive table table-striped table-bordered" 
                ...
                <thead>
                    <tr>
                        ...
                        <th class="to_hide_phone ue no_sort">bagtubuh</th>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_bagtubuh"){
				dataString = 'nama_bagtubuh='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mbagtubuh",
				type: "GET",
				data: dataString,
		...
		function pagination(page){
			...
			if (combo == "nama_bagtubuh"){
				dataString = 'starting='+page+'&nama_bagtubuh='+cari;//+'&random='+Math.random();
			}
			else if (combo == "keterangan"){
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mbagtubuh";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mbagtubuh&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) {
					...
					if(statusy=='sukses'){
						...
						if(idformx==""){ //add
							$("#myModal").modal('hide'); 
							loadData();
							$("#bagtubuh_"+idy).css("background","green");
							
						} //edit
						else{
							$("#myModal").modal('hide'); 
							$("#bagtubuh_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> bagtubuh : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mbagtubuh",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mbagtubuh&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var bagtubuhy 		= data.nama_bagtubuh;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Bagian Tubuh</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>bagtubuh</td><td> :'+ bagtubuhy+'</td></tr>';
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mbagtubuh",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#bagtubuh_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('bagtubuh : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua bagtubuh  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mbagtubuh",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_bagtubuh&idx=id_bagtubuhkerja',
				data :'tabelx=tb_bagtubuh',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_bagtubuh",
				//data: "tabelx=tb_bagtubuh&ancur="+md5,
