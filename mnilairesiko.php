...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_nilairesiko&judulx=Nilai Resiko" target="_blank">Cetak Semua</a>
			...
			<div class="span7">
			  ...
			  <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">nilairesiko </label>
                  <div class="controls span7">
                    <input id="cname" name="nama_nilairesikoTB" placeholder="nama nilairesiko (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan nilairesiko (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Nilai Resiko </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_nilairesiko" selected="selected">nama nilairesiko</option>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_nilairesiko"){
				dataString = 'nama_nilairesiko='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mnilairesiko",
				type: "GET",
				data: dataString,
		...
		function pagination(page){
			...
			if (combo == "nama_nilairesiko"){
				dataString = 'starting='+page+'&nama_nilairesiko='+cari;//+'&random='+Math.random();
			}
			else if (combo == "keterangan"){
		...
		//action klik paging
		$('li.nextpagingpaging').live('click',function(){
			var pg = $(this).attr('pg');
			pagination(pg);
			});
		...
		$('#validateForm').submit(function() {
			...
			$.ajax({
				...
				success: function(data) 
				{
					...
					var namay 	= data.nama_nilairesiko;
					...
					if(statusy=='sukses')
					{
						...
						//$("#nilairesiko_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#nilairesiko_"+idy).css("background","green");
						} //edit
						else{
							$("#nilairesiko_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> nilairesiko : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mnilairesiko",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mnilairesiko&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var nilairesikoy 		= data.nama_nilairesiko;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Nilai Resiko</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>nilairesiko</td><td> :'+ nilairesikoy+'</td></tr>';
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mnilairesiko",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#nilairesiko_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('nilairesiko : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua nilairesiko  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mnilairesiko",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_nilairesiko&idx=id_nilairesikokerja',
				data :'tabelx=tb_nilairesiko',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_nilairesiko",
				//data: "tabelx=tb_nilairesiko&ancur="+md5,
