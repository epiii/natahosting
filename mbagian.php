              ...
              <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  ...
                  <div class="controls span7">
                    <input id="cname" name="nama_bagianTB" placeholder="nama bagian (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan bagian (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
	<div class="row-fluid">
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - bagian </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_bagian" selected="selected">nama bagian</option>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_bagian"){
				dataString = 'nama_bagian='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mbagian",
				type: "GET",
				data: dataString,
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(statusy=='sukses'){
						...
						//$("#bagian_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#bagian_"+idy).css("background","green");
						} //edit
						else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mbagian",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mbagian&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var bagiany 		= data.nama_bagian;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Detail Bagian (perusahaan)</b></div>';
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua bagian  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mbagian",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_bagian&idx=id_bagiankerja',
				data :'tabelx=tb_bagian',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_bagian",
				//data: "tabelx=tb_bagian&ancur="+md5,
