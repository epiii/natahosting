...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_department&judulx=Department" target="_blank">Cetak Semua</a>
              ...
              <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  ...
                  <div class="controls span7">
                    <input id="cname" name="nama_departmentTB" placeholder="nama department (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan department (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Department </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_department" selected="selected">nama department</option>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_department"){
				dataString = 'nama_department='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mdepartment",
				type: "GET",
				data: dataString,
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mdepartment";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mdepartment&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) 
				{
					...
					var namay 	= data.nama_department;
					...
					if(statusy=='sukses')
					{
						...
						//$("#department_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#department_"+idy).css("background","green");
						} //edit
						else{
							$("#department_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> department : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mdepartment",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mdepartment&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var departmenty 		= data.nama_department;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Department</b></div>';
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mdepartment",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#department_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('department : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua department  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mdepartment",
						success: function(data){
							if(data.status=='sukses'){loadData();}
							else{alert('terjadi kesalahan pada proses database');}
						}
				});
		});
		//hapus
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_department&idx=id_departmentkerja',
				data :'tabelx=tb_department',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_department",
				//data: "tabelx=tb_department&ancur="+md5,
