...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_tempatsampling&judulx=Tempat Sampling" target="_blank">Cetak Semua</a>
			...
			<div class="span7">
			  ...
			  <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">tempatsampling </label>
                  <div class="controls span7">
                    <input id="cname" name="nama_tempatsamplingTB" placeholder="nama tempatsampling (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan tempatsampling (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Tempat Sampling </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_tempatsampling" selected="selected">nama tempatsampling</option>
            ...
            <table id="datatable_examplez" class="responsive table table-striped table-bordered" 
                ...
                <thead>
                    <tr>
                        ...
                        <th class="to_hide_phone ue no_sort">tempatsampling</th>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mtempatsampling",
				type: "GET",
				data: dataString,
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mtempatsampling";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mtempatsampling&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) 
				{
					...
					if(statusy=='sukses')
					{
						if(idformx=='')
						{
							...
							$('#myModal').modal('hide');
							loadData();
							$("#tempatsampling_"+idy).css("background","green");
						}
						else
						{
							var idy = idformx;
							$('#myModal').modal('hide');
							$("#tempatsampling_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> tempatsampling : "'+namay).fadeIn(6000);
							loadData();
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mtempatsampling",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mtempatsampling&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var tempatsamplingy 		= data.nama_tempatsampling;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Tempat Sampling</b></div>';
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mtempatsampling",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua tempatsampling  ?"))
				$.ajax({
						...
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mtempatsampling",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_tempatsampling&idx=id_tempatsamplingkerja',
				data :'tabelx=tb_tempatsampling',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_tempatsampling",
				//data: "tabelx=tb_tempatsampling&ancur="+md5,
