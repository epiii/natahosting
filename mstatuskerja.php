...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_statuskerja&judulx=Status Kerja" target="_blank">Cetak Semua</a>
			...
			<div class="span7">
			  ...
			  <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">statuskerja </label>
                  <div class="controls span7">
                    <input id="cname" name="nama_statuskerjaTB" placeholder="nama statuskerja (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Status Kerja </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_statuskerja" selected="selected">nama statuskerja</option>
					</select>
                &nbsp;
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_statuskerja"){
				dataString = 'nama_statuskerja='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mstatuskerja",
				type: "GET",
				data: dataString,
		...
		$('#validateForm').submit(function() {
			...
			$.ajax({
				...
				success: function(data) 
				{
					...
					var namay 	= data.nama_statuskerja;
					...
					if(statusy=='sukses')
					{
						...
						$("#cname2").val('');
						$('#myModal').modal('hide');
						//$("#statuskerja_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#statuskerja_"+idy).css("background","green");
						} //edit
						else{
							$("#statuskerja_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> statuskerja : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mstatuskerja",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mstatuskerja&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var statuskerjay 		= data.nama_statuskerja;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Status Kerja</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>Status Kerja</td><td> : '+ statuskerjay+'</td></tr></table>';
					//$(this).target="_blank";
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mstatuskerja",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#statuskerja_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('statuskerja : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua statuskerja  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mstatuskerja",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_statuskerja&idx=id_statuskerjakerja',
				data :'tabelx=tb_statuskerja',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_statuskerja",
				//data: "tabelx=tb_statuskerja&ancur="+md5,
