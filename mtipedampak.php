<?php 
	if ($_GET['hlm']){
		...
		if ($acak2=='mtipedampak'){
?>	
<!--<style type="text/css">
	#isi .pagination ul li.inactive,
	#isi .pagination ul li.inactive:hover{
...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_tipedampak&judulx=Tipe Dampak Lingkungan" target="_blank">Cetak Semua</a>
			...
			<div class="span7">
			  ...
			  <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">tipedampak </label>
                  <div class="controls span7">
                    <input id="cname" name="nama_tipedampakTB" placeholder="nama tipedampak (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan tipedampak (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Tipe Dampak Lingkungan </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_tipedampak" selected="selected">nama tipedampak</option>
            ...
            <table id="datatable_examplez" class="responsive table table-striped table-bordered" 
                ...
                <thead>
                    <tr>
                        ...
                        <th class="to_hide_phone ue no_sort">tipedampak</th>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_tipedampak"){
				dataString = 'nama_tipedampak='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mtipedampak",
				type: "GET",
				data: dataString,
		...
		function pagination(page){
			...
			if (combo == "nama_tipedampak"){
				dataString = 'starting='+page+'&nama_tipedampak='+cari;//+'&random='+Math.random();
			}
			else if (combo == "keterangan"){
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mtipedampak";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mtipedampak&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) 
				{
					...
					var namay 	= data.nama_tipedampak;
					...
					if(statusy=='sukses')
					{
						...
						//$("#tipedampak_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#tipedampak_"+idy).css("background","green");
						} //edit
						else{
							$("#tipedampak_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> tipedampak : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mtipedampak",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mtipedampak&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var tipedampaky 		= data.nama_tipedampak;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Tipe Dampak Lingkungan</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>tipedampak</td><td> :'+ tipedampaky+'</td></tr>';
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mtipedampak",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#tipedampak_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('tipedampak : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua tipedampak  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mtipedampak",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_tipedampak&idx=id_tipedampakkerja',
				data :'tabelx=tb_tipedampak',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_tipedampak",
				//data: "tabelx=tb_tipedampak&ancur="+md5,
