...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_gas&judulx=Jenis Gas" target="_blank">Cetak Semua</a>
              ...
              <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  ...
                  <div class="controls span7">
                    <input id="cname" name="nama_gasTB" placeholder="nama gas (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Jenis Gas </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_gas" selected="selected">nama gas</option>
				</select>
                &nbsp;
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mgas",
				type: "GET",
				data: dataString,
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mgas";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mgas&idx="+idformx;
			}
			$('#hasily').html("loading ....").show();
			$.ajax({
				type: 'POST',
				dataType:'json',
				url: urlx + urlx2,
				...
				success: function(data){
					...
					if(statusy=='sukses'){
						if(idformx==''){
							...
							var idy = data.id;
							loadData();
							$("#gas_"+idy).css("background","green");
						}
						else{
							$('#myModal').modal('hide');
							var idy = idformx;
							$("#gas_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> gas : "'+namay).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mgas",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mgas&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var gasy 		= data.nama_gas;
					var bodyy		='';
						bodyy		+='<div align=center><b>Jenis Gas</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>gas</td><td> :'+ gasy+'</td></tr></table>';
					//$(this).target="_blank";
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mgas",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#gas_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('gas : "'+namay+'",  terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua gas  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mgas",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_gas&idx=id_gaskerja',
				data :'tabelx=tb_gas',
				success:function(data){
					//opener = $('<div>').append(print);
