...
<a id="cetakBC" class="btn btn-info btn-large" href="pcetak.php?tabelx=tb_katinvestigasi&judulx=Kategori Investigasi" target="_blank">Cetak Semua</a>
			...
			<div class="span7">
			  ...
			  <form class="form-horizontal cmxform" id="validateForm" method="get" action="pmaster.php" autocomplete="off">
                ...
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3">katinvestigasi </label>
                  <div class="controls span7">
                    <input id="cname" name="nama_katinvestigasiTB" placeholder="nama katinvestigasi (wajib diisi)" type="text" required class="span12"/>
                  </div>
                </div>
                <div class="form-row control-group row-fluid">
                  <label class="control-label span3" for="hint-field">keterangan </label>
                  <div class="controls span7">
                    <input id="cname2" name="keteranganTB" minlength="3" type="text" required placeholder="keterangan katinvestigasi (wajib diisi)"class="row-fluid"/>
                  </div>
                </div>
...
<script language="javascript" type="text/javascript" src="js/plugins/avgrund.js"></script> 
        ...
        <div class="box ">
          <div class="title">
            <h4> <span>Data Master - Kategori Investigasi </span>
           </h4>&nbsp;
				<select id="kategoriTB">
					<option value="nama_katinvestigasi" selected="selected">nama katinvestigasi</option>
            ...
            <table id="datatable_examplez" class="responsive table table-striped table-bordered" 
                ...
                <thead>
                    <tr>
                        ...
                        <th class="to_hide_phone ue no_sort">katinvestigasi</th>
...
<script type="text/javascript">
	...
	{	
		...
		function loadData(){
			...
			if (combo == "nama_katinvestigasi"){
				dataString = 'nama_katinvestigasi='+ cari;
			}
			...
			$.ajax({
				url	: "pmaster.php?aksi=tampil&menu=mkatinvestigasi",
				type: "GET",
				data: dataString,
		...
		function pagination(page){
			...
			if (combo == "nama_katinvestigasi"){
				dataString = 'starting='+page+'&nama_katinvestigasi='+cari;//+'&random='+Math.random();
			}
			else if (combo == "keterangan"){
		...
		$('#validateForm').submit(function() {
			...
			if(idformx==''){ //tmbah data
				urlx2 = "?aksi=tambah&menu=mkatinvestigasi";
			}
			else{ //edit data
				urlx2 = "?aksi=ubah&menu=mkatinvestigasi&idx="+idformx;
			}
			$('#hasily').html("loading ....");
			$.ajax({
				type: 'POST',
				...
				success: function(data) 
				{
					...
					var namay 	= data.nama_katinvestigasi;
					...
					if(statusy=='sukses')
					{
						...
						//$("#katinvestigasi_"+idformx).css("background","blue").delay(2000).tampil(datax);
						if(idformx=="")
						{ //add
							loadData();
							$("#katinvestigasi_"+idy).css("background","green");
						} //edit
						else{
							$("#katinvestigasi_"+idy).css("background","blue");
							$("#loadtabel").html('update <br> katinvestigasi : "'+namay+'" <br> keterangan : '+keterangany).fadeIn(6000);
							loadData();
						}
					}else{
		...
		$('i.gicon-edit').live("click",function() {	
			...
			$.ajax({
				url:"pmaster.php?aksi=ambiledit&menu=mkatinvestigasi",
				data: "idx="+idy,
				dataType:'json',
				type:"GET",
				success:function(data){
		...
		$('i.gicon-eye-open').live("click",function() {	
			...
			$.ajax({
				...
				dataType:'json',
				url:"pmaster.php",
				data:"aksi=cetak&menu=mkatinvestigasi&idx="+idy,//+"&namax="+namay+"&keteranganx="+keterangany,
				success:function(data){
					var katinvestigasiy 		= data.nama_katinvestigasi;
					var keterangany	= data.keterangan;
					var bodyy		='';
						bodyy		+='<div align=center><b>Kategori Investigasi</b></div>';
						bodyy		+='<table align=center >';	
						bodyy		+='<tr><td>katinvestigasi</td><td> :'+ katinvestigasiy+'</td></tr>';
						bodyy		+='<tr><td>Keterangan</td><td>: '+keterangany+'</td></tr></table>';
					//$(this).target="_blank";
					//window.open($(this).prop('href'));
					//return false;//alert('hai'+data);
					var print = $('<div>', {
											id:   'cetakkatinvestigasi',
											html: bodyy,
											css:  {
												/*backgroundColor:'#fff',*/
												margin:'50px',
		...
		$('i.gicon-remove').live("click",function() {	
			...
			if(confirm("lanjutkan menghapus '"+namay+"' ?"))
				$.ajax({
						type: "GET",
						dataType:"json",
						url: "pmaster.php?aksi=hapus&menu=mkatinvestigasi",
						data:"idx=" + idy,
						cache: false,
						success: function(data){
							var statusy		= data.status;
							...
							$("#katinvestigasi_"+idy).css("background","red").fadeOut(2000);
							loadData();
							$("#loadtabel").html('katinvestigasi : "'+namay+'", keterangan : '+keterangany+' terhapus').fadeIn(6000);
		...
		$('#hapusBC').click(function() {	
			if(confirm("lanjutkan menghapus semua katinvestigasi  ?"))
				$.ajax({
						type: "GET",
						dataType:'json',
						url: "pmaster.php",
						data:"aksi=hapussemua&menu=mkatinvestigasi",
						success: function(data){
		...
		$('#cetakBCx').click(function(){
			$.ajax({
				type:'post',
				url:'lib/tespddf.php',
				//data :'tabelx=tb_katinvestigasi&idx=id_katinvestigasikerja',
				data :'tabelx=tb_katinvestigasi',
				success:function(data){
					//opener = $('<div>').append(print);
		...
		$('#cetakBCz').click(function(e) {
			...
			jQuery.ajax({
				...
				url: "lib/tespddf.php",
				data: "tabelx=tb_katinvestigasi",
				//data: "tabelx=tb_katinvestigasi&ancur="+md5,
